...
const Form = ({ currentId, setCurrentId }) => {
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ body: '', user_id: 1, date: Date.now() });
  };
 ...
 return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        ...
        <TextField name="message" variant="outlined" label="Message" fullWidth multiline minRows={10} value={postData.body} onChange={(e) => setPostData({ ...postData, body: e.target.value })} />
